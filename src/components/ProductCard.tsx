...
export function ProductCard({ product, isLoading, notFound }: Props) {
  ...
  const isActive = product.estado === 0
  const issuedDate = new Date(Number(product.emitidoEn) * 1000).toLocaleDateString('es-CO', {
  ...
  return (
    <div className={`rounded-xl border p-6 ${isActive ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
      ...
      <h2 className="mt-4 text-xl font-bold text-gray-900">{product.lineaProducto}</h2>
      ...
      <div className="mt-4 space-y-2 text-sm text-gray-600">
        ...
        <div className="flex items-center gap-2">
          ...
          <a
            href={`${EXPLORER_URL}/address/${product.emisor}`}
            target="_blank"
            rel="noopener noreferrer"
            className="font-mono text-xs text-purple-600 underline"
          >
            {product.emisor.slice(0, 6)}...{product.emisor.slice(-4)}
          </a>
        </div>
        <div className="flex items-center gap-2">
          <span className="font-medium text-gray-700">Dueño actual:</span>
          <a
            href={`${EXPLORER_URL}/address/${product.duenoActual}`}
            target="_blank"
            rel="noopener noreferrer"
            className="font-mono text-xs text-purple-600 underline"
          >
            {product.duenoActual.slice(0, 6)}...{product.duenoActual.slice(-4)}
          </a>
        </div>
        {!isActive && (
